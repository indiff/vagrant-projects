...
#
#   dbca has no notion of SEHA — it only ever creates a plain single-instance
#   database on ASM. What makes it a SEHA database is the candidate node list,
#   which only srvctl can set, so the database is not fully created until both
#   steps below have run.
#------------------------------------------------------------------------------
