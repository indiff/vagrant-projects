...
#
# setup.sh
#   Orchestrator for the Oracle SEHA two-node provisioning flow.
...
if [[ "${ORESTART}" != "false" || "${DB_TYPE}" != "SEHA" ]]; then
  echo "ERROR: SEHA requires a two-node Grid Infrastructure cluster (ORESTART=false, DB_TYPE=SEHA); Oracle Restart and RAC are not supported" >&2
  exit 1
fi
...
if [[ "${is_node1}" == "true" && "${ORESTART}" == "false" ]] \
  ...
  # (SE2 + RAC is desupported), so each node installs its own.
  #
