...
#
# 21c ships Standard Edition High Availability in its base release, so — unlike
# the 19c project, where 19.3 predates SEHA and an RU to 19.7 or newer is
# mandatory — patching here is opt-in. Set both env.opatch_software and
# env.gi_ru_software in config/vagrant.yml to enable it; leave them unset and
# every RU step below becomes a no-op that leaves the homes at base release.
#------------------------------------------------------------------------------
