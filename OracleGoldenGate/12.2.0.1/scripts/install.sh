...
unzip /vagrant/$ORACLE_DB_SETUP_FILE -d /tmp
...
su -l oracle -c "/tmp/database/runInstaller -silent -showProgress -ignorePrereq -waitforcompletion -responseFile /tmp/db_install.rsp"
